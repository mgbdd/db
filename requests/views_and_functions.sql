-- Представление для получения клиентов с невостребованными заказами
...
WHERE
    mo.status = 'ready'
AND
    mo.date_of_issue IS NULL;

-- Функция для подсчета общего числа клиентов с невостребованными заказами
...
$$ LANGUAGE plpgsql;-- Представление для получения клиентов, ожидающих поставку медикаментов
...
FROM
    ...
    prescription p ON mo.prescription_id = p.id
        JOIN
    medicine med ON p.medicine_id = med.id -- Assuming 'medicine' table contains the medication info for prescriptions
        JOIN
...
BEGIN
    RETURN QUERY
        ...
        WHERE
            m.name = med_name
          AND mo.order_date BETWEEN start_date AND end_date;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    RETURN QUERY
        SELECT DISTINCT
            ...
            top.instructions AS tech_instructions,
...
SELECT
    ...
    tp.instructions AS preparation_instructions,
