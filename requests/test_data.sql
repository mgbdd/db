DO $$
    ...
    BEGIN
        ...
        INSERT INTO technology_of_preparation (description, instructions)
        VALUES
            ('Стандартная технология приготовления', 'Смешать ингредиенты и нагреть до 80°C'),
            ('Ускоренная технология', 'Быстро перемешать с последующим охлаждением'),
            ('Натуральная технология', 'Использовать только органические компоненты, минимальное нагревание');
        SELECT COALESCE(MIN(id), 1) - 1 INTO v_tech FROM technology_of_preparation;

        -- Клиенты
        INSERT INTO client (surname, name, patronymic, phone_number)
        VALUES
            ('Иванов', 'Иван', 'Иванович', '1234567890'),
            ('Петров', 'Пётр', 'Петрович', '2345678901'),
            ('Сидоров', 'Сергей', 'Сергеевич', '3456789012'),
            ('Морозов', 'Алексей', 'Алексеевич', '4567890123'),
            ('Кузнецов', 'Дмитрий', 'Дмитриевич', '5678901234');
        SELECT COALESCE(MIN(id), 1) - 1 INTO v_client FROM client;

        /* Вставляем лекарства и получаем их ID */
        ...
        /* Вставляем ингредиенты и получаем их ID */
        ...
        */
        INSERT INTO prescription (
            medicine_id, prescription_number, doctor_surname, doctor_name, doctor_patronymic,
            signature, stamp, age, diagnosis, amount, application
        )
        VALUES
            (paracetomol_id, 1001, 'Петров', 'Сергей', 'Николаевич',
             DECODE('DEADBEEF','hex'), DECODE('CAFEBABE','hex'),
             45, 'Грипп', 5.00, 'oral'),
            (antibiotic_id, 1002, 'Смирнов', 'Александр', 'Иванович',
             DECODE('A1B2C3D4','hex'), DECODE('D4C3B2A1','hex'),
             30, 'Бактериальная инфекция', 10.00, 'injection'),
            (analgin_id, 1003, 'Козлов', 'Денис', 'Петрович',
             DECODE('11223344','hex'), DECODE('44332211','hex'),
             60, 'Болевой синдром', 3.00, 'topical'),
            (antibiotic_id, 1004, 'Новиков', 'Владимир', 'Александрович',
             DECODE('0A0B0C0D','hex'), DECODE('0D0C0B0A','hex'),
             50, 'Инфекция', 7.00, 'injection');
        ...
        VALUES
            (v_prescription + 1, v_client + 1, 101, CURRENT_DATE, 'ready', CURRENT_DATE, INTERVAL '1 hour', 25.00),
            (v_prescription + 2, v_client + 2, 102, CURRENT_DATE - INTERVAL '1 day', 'producing', NULL, NULL, 120.00),
            (v_prescription + 3, v_client + 3, 103, CURRENT_DATE - INTERVAL '2 days', 'ready', CURRENT_DATE - INTERVAL '1 day', INTERVAL '2 hours', 30.00),
            (v_prescription + 4, v_client + 4, 104, CURRENT_DATE - INTERVAL '3 days', 'waiting for a delivery', NULL, NULL, 84.00);
