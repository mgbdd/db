...
BEGIN
    ...
    IF med_type = 'finished' THEN
        IF current_quantity >= NEW.ammount THEN
        ...
        ELSE
            ...
            INSERT INTO medication_delivery (
                ...
                NEW.medicine_id,
                CURRENT_DATE,
                NEW.ammount - current_quantity
            );
        END IF;
    ELSE
        IF check_components(NEW.medicine_id, NEW.ammount) THEN
            status_val := 'producing'::order_status;
        ELSE
            status_val := 'waiting for a delivery'::order_status;
            -- Заказ недостающих компонентов
            PERFORM order_missing_components(NEW.medicine_id, NEW.ammount);
        END IF;
    END IF;
...
-- 4. Триггер для обработки поступлений
